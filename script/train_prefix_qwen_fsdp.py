...
class PadAndMaskCollator:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        enc = self.tok.pad({"input_ids": input_ids}, padding=True, max_length=self.max_len, return_tensors="pt")
        ...
        return {"input_ids": ids, "attention_mask": attn, "labels": padded_labels, "aa_seq": aa_list, "stru_str": stru_list, }
...
def train(args):
    ...
    val_loader = (DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=0, pin_memory=False, collate_fn=collate) if val_ds else None)

    # Model
    big = BigProteinQwen(model_name=args.model_name, protein_config=args.protein_config, structure_config=args.structure_config, protrek_ckpt=args.protrek_ckpt,
                         prot_slot=args.prot_slot, stru_slot=args.stru_slot, single_token_prefix=args.single_token_prefix, prefix_len=args.prefix_len, proj_hid=args.proj_hid,
                         dropout=args.dropout, train_encoders=args.train_encoders, dtype_str=args.dtype, )
    ...
    for epoch in range(args.epochs):
        for it, batch in enumerate(train_loader):
            ...
            if accelerator.is_main_process and args.save_every and (global_step % args.save_every == 0):
                ...
                state = {"llm": accelerator.get_state_dict(big.llm), "proj_1024": accelerator.get_state_dict(big.proj_1024), "proj_2048": accelerator.get_state_dict(big.proj_2048),
                         "protein_encoder": accelerator.get_state_dict(big.protein_encoder) if args.train_encoders else None,
                         "structure_encoder": accelerator.get_state_dict(big.structure_encoder) if args.train_encoders else None, "optimizer": optimizer.state_dict(),
    ...
    if accelerator.is_main_process:
        ...
        state = {"llm": accelerator.get_state_dict(big.llm), "proj_1024": accelerator.get_state_dict(big.proj_1024), "proj_2048": accelerator.get_state_dict(big.proj_2048),
                 "protein_encoder": accelerator.get_state_dict(big.protein_encoder) if args.train_encoders else None,
                 "structure_encoder": accelerator.get_state_dict(big.structure_encoder) if args.train_encoders else None, "optimizer": optimizer.state_dict(), "args": vars(args),
