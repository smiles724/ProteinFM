        ...
        with open(self.path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if "prompt" not in ex or "response" not in ex:
                    continue
                yield {"prompt": ex["prompt"], "response": ex["response"], "aa_seq": ex.get("aa_seq", None), "stru_str": ex.get("stru_str", None), }
...
class PadOnlyTextCollator:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        return {"input_ids": input_ids, "attention_mask": attn_mask, "labels": labels, "aa_seq": aa_list, "stru_str": stru_list, }
...
def load_encoders_and_ckpt(protein_config: str, structure_config: str, ckpt_path: Optional[str], prot_slot: int, stru_slot: int, device: torch.device):
...
def train(args):
    ...
    prot_enc, stru_enc = load_encoders_and_ckpt(args.protein_config, args.structure_config, args.protrek_ckpt, args.prot_slot, args.stru_slot, device)
    ...
    projector = nn.Sequential(nn.Linear(1024 + 1024, hidden_size), nn.SiLU(), nn.Linear(hidden_size, hidden_size * args.prefix_len), ).to(device, dtype=model_dtype)
    ...
    collate = PadOnlyTextCollator(CollateConfig(tokenizer=tok, max_len=args.max_len, device=device))
    ...
    autocast_ctx = (torch.amp.autocast(device_type="cuda", dtype=model_dtype) if (device.type == "cuda" and model_dtype in (torch.float16, torch.bfloat16)) else None)
    ...
    def forward_batch(batch) -> Tuple[torch.Tensor, int]:
        ...
        for i in range(B):
            if aa_list[i]:
                prot_full[i] = prot_vec_list[ip];
                ip += 1
            if stru_list[i]:
                stru_full[i] = stru_vec_list[is_];
                is_ += 1
        ...
        if prefix_gate is not None:
            pref = pref * (prefix_gate if isinstance(prefix_gate, torch.Tensor) else torch.tensor(prefix_gate, device=device, dtype=model_dtype))

        inputs_embeds = torch.cat([pref, text_embeds], dim=1)  # (B, P+T, H)
        attn_mask = torch.cat([torch.ones(B, args.prefix_len, device=device, dtype=torch.long), attn_mask_text], dim=1)
        labels = torch.cat([torch.full((B, args.prefix_len), -100, device=device, dtype=torch.long), labels_text], dim=1)
        ...
        for step, batch in enumerate(train_loader, start=1):
            ...
            if step % args.accum_steps == 0:
                ...
                if args.save_every and (global_step % args.save_every == 0):
                    ...
                    save_state = {"projector": projector.state_dict(), "llm": (llm.state_dict() if not args.freeze_llm else None),
                                  "encoders": {"protein": prot_enc.state_dict() if args.train_encoders else None,
                                               "structure": stru_enc.state_dict() if args.train_encoders else None, },
                                  "prefix_gate": (prefix_gate.data if isinstance(prefix_gate, nn.Parameter) else prefix_gate), "optimizer": optimizer.state_dict(),
                                  "args": vars(args), "global_step": global_step, "epoch": epoch, }
    ...
    if args.save_dir:
        ...
        save_state = {"projector": projector.state_dict(), "llm": (llm.state_dict() if not args.freeze_llm else None),
                      "encoders": {"protein": prot_enc.state_dict() if args.train_encoders else None, "structure": stru_enc.state_dict() if args.train_encoders else None, },
                      "prefix_gate": (prefix_gate.data if isinstance(prefix_gate, nn.Parameter) else prefix_gate), "optimizer": optimizer.state_dict(), "args": vars(args),
